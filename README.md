...
支持一个系统同时服务多场婚礼，每场婚礼数据与媒体完全隔离。

- **系统管理员**（`/admin`，config 中 `admin.user/pass`）：登录后总览所有婚礼，可「创建新婚礼」与删除。创建后为每场婚礼生成**两个独立 token**：
  - **请柬链接**（`/w/{token}`，公开）：发给宾客，只能浏览请柬并提交回执；
  - **后台链接**（`/admin/{adminToken}`，私密）：请**私发新人**，勿随请柬公开。凭它进入该婚礼后台配置内容/样式、维护来宾回执、上传背景图与音乐。
- 数据隔离：每场婚礼独立的 settings / 来宾 / 访问记录；背景图与音乐按 `web/static/{请柬token}/images|music` 分目录。
- 安全：token 为 `crypto/rand` 生成的 **10 位随机小写字母**（约 47 bit 熵）；所有查询参数化；媒体上传白名单扩展名并防路径穿越；删除婚礼级联清理数据与媒体。
- ⚠️ 请柬链接内含 token，请通过 HTTPS 访问并妥善保管后台链接。

### 快速体验（多租户）

```bash
go build -o wedding .
./wedding   # 默认 config.yml：port 8080
# 1) 打开 http://localhost:8080/admin 用 admin 登录
# 2) 创建婚礼 → 自动复制「后台链接」
# 3) 后台链接打开该婚礼后台；「复制请柬」的链接发给宾客
```
